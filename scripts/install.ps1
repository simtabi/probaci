<#
.SYNOPSIS
  probaci installer for Windows.

.EXAMPLE
  irm https://opensource.simtabi.com/install/probaci.ps1 | iex

.EXAMPLE
  # Pin a version / custom dir (env vars are prefixed with the uppercased binary name)
  $env:probaci_VERSION = 'v0.1.0'; irm https://.../install.ps1 | iex
#>
#Requires -Version 5
[CmdletBinding()]
param(
...
# project name (uppercase, non-alnum -> '_').
$prefix = ($repo.ToUpper() -replace '[^A-Z0-9]', '_')
...
# Bare, ready-to-run executable: probaci_windows_<arch>.exe.
$asset = "${repo}_windows_${arch}.exe"
...
Write-Host "Installed $repo to $InstallDir\$binary"
