...
# build/dist/:
#
#   probaci_{os}_{arch}[.exe]     flat, versionless, ready-to-run binaries
#   probaci_macos_universal       Intel + Apple Silicon fat binary (on macOS)
#   checksums.txt                   SHA-256 over the bare binaries (bare names)
#   archives/probaci_{os}_{arch}.{tar.gz,zip}   binary + LICENSE/README/CHANGELOG
...
BINARY="probaci"
PKG="github.com/simtabi/probaci/internal/version"
BUILD_DIR="build"
OUT="${BUILD_DIR}/dist"
...
make_archive() {
  ...
  bin="$1"; goos="$2"
  base="${bin%.exe}"                      # probaci_macos_arm64
  stage="$(mktemp -d)"
...
while IFS=$' \t\r' read -r goos goarch goarm || [ -n "${goos:-}" ]; do
  ...
  GOOS="$goos" GOARCH="$goarch" GOARM="${goarm:-}" CGO_ENABLED=0 \
    go build -trimpath -ldflags "$LDFLAGS" -o "${OUT}/${name}" ./cmd/probaci
  echo "ok"
